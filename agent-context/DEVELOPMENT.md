...
| CMS collections | `Treatments`, `Conditions`, `SiteSettings`, `Locations`, `InsuranceProviders`, `Pricing`, `Testimonials`, `BookableServices`, `CalendarConfig`, `BusinessHours`, `Closures`, `BookingSettings`, `BookingEmails` (ADMIN-read), `Appointments` (ADMIN-read) |
...
- **Booking emails:** on success the server inserts a row into the **`BookingEmails`** CMS collection (`src/lib/booking/confirmation.ts`). Wix Automations can use the same **"Item added" to BookingEmails** trigger for two actions: (1) send the patient confirmation using `email`/`firstName`/`service`/`location`/`appointmentTime`/`referenceId`, and (2) notify `marinholyhillacu@gmail.com` using `patientName`/`email`/`patientPhone`/`patientType`/`service`/`location`/`appointmentTime`/`reservationCreatedAt`/`referenceId`. A real CMS collection is used because every value is available to the Automation as a collection field. DOB, insurance, and free-text messages remain only in the ADMIN `Appointments` collection and are not emailed. (The old `BOOKING_CONFIRMATION_FORM_ID` in `config.ts` is deprecated/unused.)
