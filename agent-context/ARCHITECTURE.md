...
- CMS collections (`Treatments`, `Conditions`, `SiteSettings`, `Locations`, `InsuranceProviders`, `Pricing`, `Testimonials`, plus the booking collections `BookableServices`, `CalendarConfig`, `BusinessHours`, `Closures`, `BookingSettings`, `BookingEmails`, `Appointments`) are represented by typed guarded adapters and the idempotent migration in `scripts/wix-seed.mjs`.
- **Direct booking (Book Directly):** the `/book` page runs a React wizard backed by Astro API routes (`src/pages/api/booking/*`) that talk to **Google Calendar** (source of truth for availability, via a **git-ignored** service-account key + Web Crypto JWT — server-only; see `BOOKING_CREDENTIALS.md`) and **Wix CMS** (config + the ADMIN-read `Appointments` record). Patient confirmation and clinic notification emails are driven by a Wix Automation when the server inserts the post-booking `BookingEmails` CMS record. Full spec: `agent-context/WIP/agent_21/PLAN.md`; developer notes: `DEVELOPMENT.md` §7b. Zocdoc is an optional secondary link: clearing `SiteSettings.bookingUrl` removes it from the rendered site.
